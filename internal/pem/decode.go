...
// Certificates have two variables that we can esimate easily; the public key of the cert, and the signature from the signing cert.
...
// SafeDecodeMultipleCertificates is an alias for SafeDecodeCertificateChain.
// DEPRECATED: Use SafeDecodeCertificateChain or SafeDecodeCertificateBundle to be more descriptive
func SafeDecodeMultipleCertificates(b []byte) (*stdpem.Block, []byte, error) {
	return SafeDecodeCertificateChain(b)
}
